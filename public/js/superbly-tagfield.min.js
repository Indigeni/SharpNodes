...
function addItem(value){
	...
	inputItem.before("<li class='superblyTagItem'><span>"+value+"</span><a> x</a></li>");
   	...
   	getCountryDetailsBySite(value);
	
	tagInput.val("");
	currentValue=null;
	currentItem=null;
	var new_index=tagList.children('.superblyTagItem').size()-1;
	$(tagList.children('.superblyTagItem')[new_index]).children('a').click(function(e){var value=$($(this).parent('.superblyTagItem').children('span')[0]).text();removeItem(value);});}
...
function removeItem(value){
   	eventMan.pub("country_removed", {countryName : value}); //QUI HO RIMOSSO IL COUNTRY
...
function getSuggestionsArray(value){var suggestions=new Array();var count=0;for(var key=0; key < tagstmp.length; key++){if(showTagsNumber<=count){break;}
...
currentValue=value;suggestList.empty();var suggestions=getSuggestionsArray(value);for(var key=0; key < suggestions.length; key++){suggestList.append("<li class='superblySuggestItem'>"+suggestions[key]+"</li>");}
